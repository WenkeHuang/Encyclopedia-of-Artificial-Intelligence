...
$$
设原空间为\mathcal{X}	\sub R^2,x = (x^{(1)},x^{(2)}) \in \mathcal{X}\\新空间为Z \sub R^2，z = (z^{(1)},z^{(2)}) \in Z\\
定义映射：\\
...
在线性SVM的对偶问题中，目标函数和决策函数都只涉及输入实例与实例之间的内积。对偶问题的目标函数中的内积$x_i \cdot x_j$可以用核函数¥K(xi,xj)=ϕ(x_i)⋅ϕ(x_j)$代替。
