...
即神经网络的参数定义了一个编码函数f(x(i))f(x^{(i)})f(x(i))，如果给定输入图像x(i)x^{(i)}x(i)，这个网络会输出x(i)x^{(i)}x(i)的128维的编码。然后要做的就是学习参数

- 使得如果两个图片x(i)x^{( i)}x(i)和x(j)x^{( j)}x(j)是同一个人，那么得到的两个编码的距离就小
- 如果x(i)x^{(i)}x(i)和x(j)x^{(j)}x(j)是不同的人，那么编码距离就大
