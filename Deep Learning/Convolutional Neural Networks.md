...
如上图所示，针对5*5卷积的这一层，不包含1*1的卷积模块时，参数量为$5*5*256*64=409600$个参数，连接数为$(5 \times 5 \times 256+1)\times 64*n*n=409664*n*n个。而包含1*1的卷积模块时，参数量为1*1*256*32+5*5*32*64=59392个参数，连接数为（（1*1*256+1）*32*n*n+(5*5*32+1)*64*n*n）=59488*n*n个。由此可见，包含1*1的卷积模块比不包含的参数数量和计算量要少很多。
$$
\times
$$
