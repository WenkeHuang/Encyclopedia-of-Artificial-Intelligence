...
我们用一个四维向量来表示**man**，我们用来表示，不过在这节视频中我们先把它（上图编号1所示）称为，而旁边这个（上图编号2所示）表示**woman**的嵌入向量，称它为，对**king**和**queen**也是用一样的表示方法。在该例中，假设你用的是四维的嵌入向量，而不是比较典型的50到1000维的向量。这些向量有一个有趣的特性，就是假如你有向量和，将它们进行减法运算，即
